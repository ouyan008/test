...
public class PriorityQueueImpl3<T> implements PriorityQueue<T> {
	
	Node<T> head = null;
	
	public PriorityQueueImpl3 (int s) {
	...
	public boolean add(T value, int priority) {
		...
		if (head == null) {
			...
			while (currentNode != null) {									
				System.out.println("yyy" + currentNode.priority);
				if (priority <= currentNode.priority){
					System.out.println("zzz" + currentNode.priority + "  " + priority);
